...
m_process_new(int pid, FILE **fp)
{
  ...
  m_file_new(consts.str._stdin, R1, fp[0]);
  vm_assign(3, R0);
  m_file_new(consts.str._stdout, R2, fp[1]);
  vm_assign(4, R0);
  m_file_new(consts.str._stderr, R2, fp[2]);
  vm_assign(5, R0);

  vm_inst_alloc(process_consts.cl.process);
...
cm_process_new(unsigned argc, obj_t args)
{
  ...
  argv_size = (list_len(arg) + 1) * sizeof(char *);
  argv = (char **) _cmalloc(argv_size);
  for (r = argv, p = arg; p; p = CDR(p), ++r) {
    *r = STRING(CAR(p))->data;
  }
  *r = 0;
...
cm_process_pid(unsigned argc, obj_t args)
{
  ...
  m_integer_new(PROCESS(recvr)->pid);
}
